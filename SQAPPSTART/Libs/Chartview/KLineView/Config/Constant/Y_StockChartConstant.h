 ...
 *  K线图最小的高度
 */
#define Y_StockChartKLineMinHeight 1
 ...
 *  K线的缩放因子
 */
#define Y_StockChartScaleFactor (1.0 / Y_StockChartKLineMinCount + 0.01)
 ...
 *  时分线的宽度
 */
#define Y_StockChartTimeLineLineWidth 1
 ...
 *  时分线的成交量的线宽
 */
#define Y_StockChartTimeLineVolumeLineWidth 1
 ...
 *  K线图上可画区域最大的Y
 */
#define Y_StockChartKLineMainViewMaxY (self.frame.size.height - 25)

/**
 *  K线图上时间纵坐标位置
 */
#define Y_StockChartKLineTimeY (self.frame.size.height - 15)

/**
 *  K线图上时间显示个数
 */
#define Y_StockChartKLineTimeNum (NSInteger)([UIScreen mainScreen].bounds.size.width / 60)

/**
 *  K线图上时间间隔
 */
#define Y_StockChartKLineTimeSpacing (self.frame.size.width / 6)
...
//Kline全屏样式
typedef NS_ENUM(NSInteger, Y_StockChartScreenStyle) {
    Y_StockChartScreenStyleHalf= 1, //半屏
    Y_StockChartScreenStyleFull,  //全屏
    Y_StockChartScreenStyleOther
};
