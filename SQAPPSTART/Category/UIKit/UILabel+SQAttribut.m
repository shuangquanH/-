...
- (void)appendImage:(UIImage    *)image withIndex:(NSInteger)index {
    NSMutableAttributedString *mustring = (self.attributedText)?self.attributedText.mutableCopy:[[NSMutableAttributedString alloc] initWithString:self.text];
    [self labelAppendImage:image withIndex:index mutubleString:mustring];
    self.attributedText = mustring;
}
...
- (void)labelAppendImage:(UIImage *)image withType:(SQAppendImageType)type mutubleString:(NSMutableAttributedString *)mustring {
    if (type==SQAppendImageInLeft) {
        [self labelAppendImage:image withIndex:0 mutubleString:mustring];
    } else {
        [self labelAppendImage:image withIndex:mustring.length mutubleString:mustring];
    }
}
- (void)labelAppendImage:(UIImage *)image withIndex:(NSInteger)index mutubleString:(NSMutableAttributedString *)mustring {
    //创建NSTextAttachment
    NSTextAttachment *attachment = [[NSTextAttachment alloc] init];
    attachment.image = image;
    ...
    [mustring insertAttributedString:attachmentString atIndex:index];
}
